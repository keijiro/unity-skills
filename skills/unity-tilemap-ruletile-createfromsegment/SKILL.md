...
This skill creates Unity RuleTile TilingRules by first analyzing sprites using the `unity-sprite-segment-3x3grid` skill, then converting the resulting text patterns into neighbor rule configurations. The user must specify Sprites or a Spritesheet input.

## Workflow

```
Step 1: unity-sprite-segment-3x3grid    Step 2: Create TilingRules
+--------------------------+      +--------------------------+
...
**CRITICAL**: Always run the `unity-sprite-segment-3x3grid` skill first on each Sprite to generate the text pattern.

## Step 1: Apply unity-sprite-segment-3x3grid (Required)

For each Sprite in your texture, analyze it with `unity-sprite-segment-3x3grid`:
...
1. **For each Sprite**: Run `unity-sprite-segment-3x3grid` and store the pattern
...
- Always run `unity-sprite-segment-3x3grid` first -- this skill depends on its output
...
- **`unity-sprite-segment-3x3grid` skill**: generates the input patterns. It ships in this
