...
- **`unity-urp-postprocessing`**, **`unity-ui`**, **`unity-2d-pixel-perfect`** — the visual baseline (Step 6):
  post-processing volume setup, HUD framework choice, pixel-perfect 2D.
- **`unity-implement-in-app-purchases`**, **`levelplay-unity-integration`**, **`unity-build-live-game`** —
...
the `unity-urp-postprocessing` volume framework and every `Universal Render Pipeline/*` shader are URP
...
names are `Standard` / `Unlit/Color`. Don't reach for `unity-migrate-birp-to-urp` — the user asked for
Built-in.
...
1. **Post-processing.** Global Volume with Tonemapping (ACES), low Bloom (intensity 0.5–1,
   ...
   camera's `UniversalAdditionalCameraData`. **REQUIRED SUB-SKILL:** `unity-urp-postprocessing` — its
...
4. **UI stack.** HUD and menus use a uGUI Canvas + TextMeshPro or UI Toolkit — **never `OnGUI`**.
   **REQUIRED SUB-SKILL:** `unity-ui` picks between them for the project.
...
- IAP → **unity-implement-in-app-purchases**
- Ads → **levelplay-unity-integration**
- Accounts / cloud save / economy / remote config / leaderboards → **unity-build-live-game**
